...
---
title: 'Factor Analysis: PCA vs Autoencoder'
...
- \definecolor{framecolor}{RGB}{163, 21, 21}
- \definecolor{backcolor}{RGB}{220, 220, 220}
- \definecolor{backcolor2}{RGB}{237, 237, 237}
- \usepackage{hyperref}
- \urlstyle{same}
- \usepackage{eurosym}
- \usepackage{float}
...
- \renewcommand{\schemename}{Figura}
- \DeclareCaptionType[fileext=los,placement={!ht}]{troll} 
- \renewcommand{\trollname}{Tabella}
...
- \newtcolorbox[auto counter, list inside=boxes]{mybox}[1][]{breakable, colframe=framecolor, colback=black!5!white,title=Box \thetcbcounter :, fonttitle=\normalsize\bfseries, subtitle style= {top=-2pt, nobeforeafter,colback=black!5!white, coltext=black, colframe=black!5!white, fontupper=\itshape\small}, parbox=false, arc=3mm, sharp corners=downhill}
classoption: table
# abstract: ""
---
...
The target variables is *medv*, while the others can be used as explanatory variables. Table \ref{tab:tabella} and Figures \ref{box_1}-\ref{box_2} show that the feature *black* has up to 15% of outliers, while *zn* and *crime* have about 13%. Figure \ref{corr} shows that the features that are most correlated with *medv* are *lstat*, *ptratio*, *tax*, *rm*, *nox*, *indus*. 
...
\begin{scheme}
\caption{\textbf{Boxplot of features}}
\label{box_1}
```{r box_1,echo=FALSE, warning = FALSE, fig.width=10, fig.height=12, fig.show='hold',fig.align='center'}
...
\caption{\textbf{Boxplot of features}}
\label{box_2}
```{r box_2,echo=FALSE, warning = FALSE, fig.width=10, fig.height=12, fig.show='hold',fig.align='center'}

par(mfrow = c(3, 2), mar = c(2, 2, 2, 2))
for(i in 10:ncol(boston)){
...
\label{corr}
\vspace{-1em}
```{r corr,echo=FALSE, warning = FALSE, fig.width=7, fig.height=6, fig.show='hold',fig.align='center'}
...
```{r ,echo=T, warning = F, include=T, results=F, eval  = F, fig.cap="Feature selection with BIC"}
...
\label{hist_1}
```{r hist_1,echo=FALSE, warning = FALSE, fig.width=10, fig.height=12, fig.show='hold',fig.align='center'}
...
\label{qq}
```{r qq,echo=FALSE, warning = FALSE, fig.width=10, fig.height=12, fig.show='hold',fig.align='center'}
...
\label{qq_pca}
```{r qq_pca,echo=FALSE, warning = FALSE, fig.width=10, fig.height=12, fig.show='hold',fig.align='center'}
...
The singular values $\lambda_p$ are shown below.
...
Figure \ref{arch} lhs shows the architecture of the neural network. Bias units have not been included since the features are already standardized, which results in a BNN with 98 estimated parameters. Figure \ref{arch} rhs shows the decrease of Frobenius loss in the gradient descent algorithm. Training the BNN over 5000 epochs results in a reconstruction error of about 0.95, which is much smaller than the one obtained with PCA and marked by the red horizontal line. 
...
weights = c(unlist( get_weights(model) ), rep(0.001, q0 + q1 + q2 + q1))
plotnet(weights,struct=struct, bias = F, nid = F, node_labs = F, bord_col = "black", circle_col = "lightgray")

fit <- model %>% fit(as.matrix(dati_norm), as.matrix(dati_norm), 
                       epochs=5000, batch_size=nrow(dati_norm), verbose=0)

plot(x=c(1:length(fit[[2]]$loss)), y=sqrt(fit[[2]]$loss*q0),  
     ylim=c(0,max(sqrt(fit[[2]]$loss*q0))),
     pch=19, cex=.5, 
...
The BNN architecture for the outer and inner part is shown in Figure \ref{outer}.


\begin{scheme}
\caption{\textbf{BNN architecture for outer and inner part}}
\label{outer}
```{r outer, echo=FALSE, warning = FALSE, out.width='.49\\textwidth', fig.width=5, fig.height=5,fig.show='hold',fig.align='center'}

model_1 <- BNN_1(q0, q1)
struct<-c(q0, q1, q0) 
weights = c(unlist( get_weights(model_1) ), rep(0.001, q0 + q1))
plotnet(weights,struct=struct, bias = F, nid = F, 
         node_labs = F, bord_col = "black", circle_col = "lightgray")

model_2 <- BNN_1(q1, q2)
struct<-c(q1, q2, q1) 
weights = c(unlist( get_weights(model_2) ), rep(0.001, q1 + q2))
plotnet(weights,struct=struct, bias = F, nid = F, 
        node_labs = F, bord_col = "black", circle_col = "lightgray")
...
Figure \ref{pre_train} shows the decrease of the Frobenius loss over 5000 training epochs. We see that after about 2000 epochs the reconstruction error falls below the one of the PCA with 2 principal components. 

\begin{scheme}
\caption{\textbf{Loss function of pre-trained BNN}}
\label{pre_train}
```{r pre_train, echo=FALSE, warning = FALSE, out.width='.49\\textwidth', fig.width=5, fig.height=5.5,fig.show='hold',fig.align='center'}

fit <- model %>% fit(as.matrix(dati_norm), as.matrix(dati_norm), epochs=5000, batch_size=nrow(dati_norm), verbose=0)

plot(x=c(1:length(fit[[2]]$loss)), y=sqrt(fit[[2]]$loss*q0),  
     ylim=c(0,max(sqrt(fit[[2]]$loss*q0))), pch=19, cex=.5, 
     xlab='epochs', ylab='Frobenius norm loss', main="gradient descent algorithm") 
abline(h=c(reconstruction[2]), col="orange")

```
\vspace{-0.7em}
\end{scheme}

Figure \ref{clusters} shows a comparison of the clusters obtained with PCA and BNN. We see that the dimension reduction of the BNN appears to be a slightly rotated and scaled version of the PCA results. 

\begin{scheme}
\caption{\textbf{Cluster analysis}}
\label{clusters}
```{r clusters,echo=FALSE, warning = FALSE, fig.width=10, fig.height=5.5, fig.show='hold',fig.align='center'}
